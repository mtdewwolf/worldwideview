...
import {
 Trash2, RefreshCw, Download, PowerOff, Power,
} from "lucide-react";
import { PluginIcon } from "@/components/common/PluginIcon";
import { pluginManager } from "@/core/plugins/PluginManager";
import { trackEvent } from "@/lib/analytics";
import { isPluginInstallEnabled } from "@/core/edition";
import { BrowseLink } from "./PluginsTabBrowseLink";
import {
    TrustBadge,
    getTrust,
    getIcon,
...
export function PluginsTab() {
    ...
    const renderPluginItem = (record: PluginRecord, disabled: boolean) => {
        const managed = pluginManager.getPlugin(record.pluginId);
        const managedIcon = managed
            ? (typeof managed.plugin.icon === "string" ? managed.plugin.icon : "📦")
            : undefined;

        return (
          <div key={record.pluginId} className={`plugin-item${disabled ? " plugin-item--disabled" : ""}`}>
            <span className="plugin-item__icon">
              <PluginIcon icon={getIcon(record, managedIcon)} size={18} />
            </span>
            <div className="plugin-item__info">
              <div className="plugin-item__header">
                <span className="plugin-item__name">{getName(record, managed?.plugin.name)}</span>
            ...
            <div className="plugin-item__actions">
              {!disabled && updates[record.pluginId] ? (
                <button
    ...
    return (
      <div className="plugins-tab">
        ...
        <div className="plugins-tab__list">
          {enabledPlugins.map((record) => renderPluginItem(record, false))}
          {disabledPlugins.length > 0 && (
            <>
              <div className="plugins-tab__section-title">Disabled</div>
              {disabledPlugins.map((record) => renderPluginItem(record, true))}
            </>
          )}
        </div>
        {canInstall && (
